...
/**
 * @copyright Copyright (c) 2017 Macolic Sven <macolic.sven@gmail.com>
 * @license MIT.
 */
...
class PDOProcedureHelper {
         ...
         */
         public function select($dataObject, $type = false) {
                ...
                $concat = property_exists($do, 'concat') ? $do->concat : '';
                ...
                $query->bindParam(2, $tables, PDO::PARAM_STR);
                $query->bindParam(3, $fields, PDO::PARAM_STR);
                $query->bindParam(4, $joins, PDO::PARAM_STR);
                $query->bindParam(5, $condition, PDO::PARAM_STR);
                $query->bindParam(6, $group, PDO::PARAM_STR);
                $query->bindParam(7, $order, PDO::PARAM_STR);
                $query->bindParam(8, $limit, PDO::PARAM_STR);
                $query->bindParam(9, $concat, PDO::PARAM_STR);
         ...
         */
         private static function parseInsertValues($values) {
                 ...
                 while(list($key, $val) = each($values)):
                    $crumbs .= is_numeric($val) || preg_match('/NOW/', $val) ? $val.',' : '\''.$val.'\',';
                 endwhile;
                 return substr($crumbs, 0, - 1);
         }
         /**
         ...
         */
         private static function parseUpdateFields($fields) {
                 ...
                 while(list($key, $val) = each($fields)):
                    $crumbs .= $key.' = '.(is_numeric($val) || preg_match('/NOW/', $val) ? $val.',' : '\''.$val.'\',');
                 endwhile;
                 return substr($crumbs, 0, -1);
         }
         /**
         ...
         private static function parseCondition($condition, $operator, $fromJoin = false) {
                 ...
                 while($crumbs = each($condition)):
                   ...
                   while(list($k, $val) = each($crumbs['value'])):
                    $collect .= $k == 2 ? (is_numeric($val) || (preg_match('/(\.|SELECT|PASSWORD|NOW|NULL|MAX|^\(|\)$)/', $val) && !preg_match('/\@/', $val)) ? $val.' ' : '\''.$val.'\' ') : $val.' ';
                   endwhile;
